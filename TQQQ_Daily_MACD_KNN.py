# -*- coding: utf-8 -*-
"""
Created on Tue Dec 26 12:59:32 2023
...
# Create a 'Signal' column that is '1' where the MACD crosses above the Signal Line (bullish crossover)
# and '-1' where the MACD crosses below the Signal Line (bearish crossover)
df['Signal'] = 0
df.loc[df['MACD'] > df['Signal_Line'], 'Signal'] = 1
df.loc[df['MACD'] < df['Signal_Line'], 'Signal'] = -1

# Drop the NaN values that are generated by the ewm function
df = df.dropna()
...
plt.plot(Cumulative_Strategy_returns, color='g', label = 'MACD with KNN Algo Returns')
plt.legend()
plt.show()
